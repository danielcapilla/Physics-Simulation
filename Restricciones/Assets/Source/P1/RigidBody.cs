...
public class RigidBody : MonoBehaviour, ISimulable
{
    ...
    public void GetForce(VectorXD force)
    {
        // TO BE COMPLETED
        // Gravedad
        force[index] += Manager.Gravity.x * Mass;
        force[index+1] += Manager.Gravity.y * Mass;
        force[index+2] += Manager.Gravity.z * Mass;
        // Amortiguamiento lineal
        force[index] += -(Damping*Mass) * m_vel.x;
        force[index + 1] += -(Damping * Mass) * m_vel.y;
        force[index + 2] += -(Damping * Mass) * m_vel.z;

        // Amortiguamiento angular
        // M*W´= sumT - wX(M*w)
        Vector3 torque = -Vector3.Cross(m_omega,Utils.ToVector3(m_inertia*Utils.ToVectorXD(m_omega)));

        force[index +3] += -(Damping * Mass) * m_omega.x;
        force[index + 4] += -(Damping * Mass) * m_omega.y;
        force[index + 5] += -(Damping * Mass) * m_omega.z;

        force[index + 3] += torque.x;
        force[index + 4] += torque.y;
        force[index + 5] += torque.z;
    ...
    public void GetForceJacobian(MatrixXD dFdx, MatrixXD dFdv)
    {
        ...
        dFdv[index + 3, index + 3] += -(Damping * Mass);
        dFdv[index + 4, index + 4] += -(Damping * Mass);
        dFdv[index + 5, index + 5] += -(Damping * Mass);
    }
